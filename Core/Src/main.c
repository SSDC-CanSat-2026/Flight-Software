...
static void MX_GPIO_Init(void) {
  ...
  HAL_GPIO_WritePin(GPIOC,
                    STEP_DIR0_Pin | DEBUG_0_Pin | DEBUG_1_Pin | CAM0_CTRL_Pin,
                    GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOA,
                    DEBUG_2_Pin | DRV_DIR0_Pin | CAM1_CTRL_Pin | GPS_WAKE_Pin,
                    GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(
      GPIOB, IMU_nCS_Pin | SD_nCS_Pin | BMP_nCS_Pin | GPS_RST_Pin | USR_LED_Pin,
      GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : IMU_nCS_Pin SD_nCS_Pin BMP_nCS_Pin GPS_RST_Pin
                           USR_LED_Pin */
  GPIO_InitStruct.Pin =
      IMU_nCS_Pin | SD_nCS_Pin | BMP_nCS_Pin | GPS_RST_Pin | USR_LED_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
...
//    BQ28Z610_ReadVoltage(&hi2c3, &voltage);
    ...
    FRESULT result =
        write_SD(testing_data, testing_length, "debug.csv", (FA_WRITE));
...
void StartSendTelemetry(void const *argument) {
  ...
  for (;;) {
    ...
    str_len = sprintf(
        telemetry_string,
        "%d,%s,%ld,%c,%s,%3.1f,%.1f,%.1f,%.1f,%d,%d,%d,%d,%d,%d,%d,%s,%.1f,%."
        "4f,%.4f,%d,%s",
    ...
    char frame[200];
    uint16_t data_len = 0;
    xbee_status_t status = xbee_send_api_packet(
        &telemetry_string[0], str_len, &frame[0], sizeof(frame), &data_len);
    if (status != XBEE_OK) {
      osSemaphoreRelease(globalDataHandle);
      continue;
    }
