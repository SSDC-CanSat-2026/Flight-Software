...
void LC76G_init(UART_HandleTypeDef* huart)
{
    ...
    HAL_UART_Transmit(huart, LC76_DISABLE_GGL, strlen(LC76_DISABLE_GGL), TIMEOUT);
    HAL_UART_Receive(huart, NULL, 32, TIMEOUT);

    HAL_UART_Transmit(huart, LC76_DISABLE_GSA, strlen(LC76_DISABLE_GSA), TIMEOUT);
    HAL_UART_Receive(huart, NULL, 32, TIMEOUT);

    HAL_UART_Transmit(huart, LC76_DISABLE_GSV, strlen(LC76_DISABLE_GSV), TIMEOUT);
    HAL_UART_Receive(huart, NULL, 32, TIMEOUT);

    HAL_UART_Transmit(huart, LC76_DISABLE_RMC, strlen(LC76_DISABLE_RMC), TIMEOUT);
    HAL_UART_Receive(huart, NULL, 32, TIMEOUT);

    HAL_UART_Transmit(huart, LC76_DISABLE_VTG8, strlen(LC76_DISABLE_VTG8), TIMEOUT);
    HAL_UART_Receive(huart, NULL, 32, TIMEOUT);

    // Enable GGA messages.
    HAL_UART_Transmit(huart, LC76_ENABLE_GGA, strlen(LC76_ENABLE_GGA), TIMEOUT);
    HAL_UART_Receive(huart, NULL, 32, TIMEOUT);
...
LC76G_gps_data* LC76G_read_data(UART_HandleTypeDef* huart){
	HAL_UART_Transmit(huart, gps_buffer, 512, TIMEOUT);
