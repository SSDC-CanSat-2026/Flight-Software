...
void ICM42688P_read_data(ICM42688P_AccelData *data)
{
    ...
    data->accel_p 	= (data->gyro_p - data->gyro_old_p) / ((float)(curr_time - data->old_time_tick) / configTICK_RATE_HZ);
    data->accel_yaw 	= (data->gyro_y - data->gyro_old_y) / ((float)(curr_time - data->old_time_tick) / configTICK_RATE_HZ);
    data->accel_r 	= -((data->gyro_r - data->gyro_old_r) / ((float)(curr_time - data->old_time_tick) / configTICK_RATE_HZ));
